...
from workflow.fasttree_marker_analysis.random_b_run_fasttree import FastTreeMarkerAnalysisRunRandomFastTree
from workflow.model.luigi import LuigiTask, LocalTargetTree
from workflow.util.log import log


class FastTreeMarkerAnalysisDecorateRandomFastTree(LuigiTask):
    target_pct = luigi.FloatParameter()
    batch_id = luigi.IntParameter()

    def requires(self):
        return {
            'tree': FastTreeMarkerAnalysisRunRandomFastTree(batch_id=self.batch_id, target_pct=self.target_pct),
            'meta': GtdbMetadataR207()
        }

    def output(self):
        return LocalTargetTree(
            os.path.join(DIR_OUT_FASTTREE_MARKER_ANALYSIS_RANDOM, f'{self.batch_id}_{self.target_pct}',
                         'phylorank', 'fasttree_decorated.tree'))

    def run(self):
        log(f'Decorating FastTree bac (random) (batch_id={self.batch_id}) (pct={self.target_pct})',
            title=True)
        ...
        log('Creating taxonomy file')
        output_dir = os.path.dirname(self.output().path)
        tax_file_path = os.path.join(output_dir, 'taxonomy.tsv')
