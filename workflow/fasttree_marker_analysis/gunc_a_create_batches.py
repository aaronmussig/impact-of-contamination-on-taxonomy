...
from workflow.config import DEBUG, DIR_OUT_FASTTREE_MARKER_ANALYSIS
...
class FastTreeMarkerAnalysisBatchesGunc(LuigiTask):
    ...
    def requires(self):
        return {
            ...
            'msa': GtdbMsaBacR207(),
            'contigs_removed': GenomePctCongruenceContigsRemoved(target_pct=self.target_pct, congruence=self.congruence)
        }
    ...
    def run(self):
        log(f'FastTreeMarkerAnalysisBatchesGunc(c={self.congruence}, p={self.target_pct})', title=True)
        self.make_output_dirs()

        log('Loading contigs to be removed')
        ...
        log('Creating queue')
        queue = list()
        for gid in sorted(df_max_css.index):

            # Only interested in GTDB bacterial representatives which failed
            if gid not in rep_gids:
                continue

            row_contigs_removed = df_contigs_removed.loc[gid]
            pct_removed = float(row_contigs_removed['pct_removed'])
            contigs_removed = frozenset(row_contigs_removed['contigs_removed'].split('|'))

            queue.append((gid, pct_removed, contigs_removed, self.congruence, self.target_pct))
...
def worker(job):
    gid, pct_removed, contigs, congruence, target_pct = job
    results = {
        'gid': gid,
    }
