...
from workflow.config import DIR_OUT_SENTINEL, DIR_OUT_BATCH, DEBUG, PATH_GUNC_PRO_REF_DB, \
    DIR_CACHE
...
def batch_worker(batch_path):
    ...
    if DIR_CACHE is not None:
        path_gunc_db = os.path.join(DIR_CACHE, 'gunc_db_progenomes2.1.dmnd')
        os.makedirs(DIR_CACHE, exist_ok=True)
        if not os.path.isfile(path_gunc_db):
            log(f'Copying {PATH_GUNC_PRO_REF_DB} to {path_gunc_db}')
            copy_file(PATH_GUNC_PRO_REF_DB, path_gunc_db, checksum=True)
            log('Done.')
    else:
        path_gunc_db = PATH_GUNC_PRO_REF_DB
    ...
    with open(batch_path) as f:
        for line in f.readlines():
            ...
            gunc_dir = os.path.join(prodigal_dir.parent.absolute(), 'gunc_pro')
    ...
    with tempfile.TemporaryDirectory() as f_out, tempfile.TemporaryDirectory() as f_tmp:
        ...
        move_gunc_directory(f_out, d_gid_mapping, db='progenomes_2.1')
        return
...
def get_path_worker(gid):
    ...
    if not gunc_are_all_files_present(gid, gunc_dir_pro, db='progenomes_2.1'):
        return os.path.join(gid_root, 'prodigal', f'{gid}.faa')
    return None


class RunGuncOnProGenomesUsingR207(LuigiTask):
    """Runs GUNC on the ProGenomes using RQ."""
    ...
    def run(self):
        log('Running GUNC on ProGenomes using R207...', title=True)
