...
from workflow.config import DEBUG, DIR_OUT_V2_PPLACER_MARKER_SPLIT
...
class V2PplacerMarkerSplitCreateMsa(LuigiTask):
    ...
    def run(self):
        log(f'V2PplacerMarkerSplitCreateMsa(remove_pct={self.remove_pct})', title=True)
        ...
        queue = list()
        queued_gids = set()
        for gid, row in df_max_css[df_max_css.index.isin(bac_non_rep_fail)].iterrows():
            queued_gids.add(gid)
