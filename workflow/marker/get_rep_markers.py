...
from workflow.config import DEBUG, DIR_OUT_SENTINEL, DIR_OUT_MARKER_REP
...
class GetRepMarkers(LuigiTask):
    ...
    def run(self):
        log('Getting markers for GTDB R207 representative genomes', title=True)
        self.make_output_dirs()
        os.makedirs(DIR_OUT_MARKER_REP, exist_ok=True)

        log('Loading metadata')
        rep_gids = self.load_reps()

        # rep_gids = rep_gids[0:100]
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for file_name in os.listdir(tmp_dir_unaln):
                ...
                path_aln_out = os.path.join(DIR_OUT_MARKER_REP, f'{marker}.faa')
