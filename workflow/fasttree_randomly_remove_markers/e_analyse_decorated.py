...
from workflow.config import DIR_OUT_FASTTREE_RANDOMLY_REMOVE_MARKERS, DEBUG
...
from workflow.model.luigi import LuigiTask, LocalTargetTree, LocalTargetTsv
from workflow.util.log import log


class FastTreeRandomlyRemoveMarkersAnalyseDecorated(LuigiTask):
    ...
    def requires(self):
        return {
            'decorated': FastTreeRandomlyRemoveMarkersDecorateFastTree(target_pct=self.target_pct,
                                                                       replicate_id=self.replicate_id),
            'meta': GtdbMetadataR207()
        }

    def output(self):
        return LocalTargetTsv(os.path.join(self.root_dir, 'fasttree_decorated_analysed.tsv'))

    def run(self):
        log(f'FastTreeRandomlyRemoveMarkersAnalyseDecorated(p={self.target_pct}, r={self.replicate_id})', title=True)
