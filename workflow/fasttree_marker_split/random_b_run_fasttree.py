...
class FastTreeMarkerSplitRandomRunFastTree(LuigiTask):
    ...
    def requires(self):
        return {
            'msa': FastTreeMarkerSplitRandomCreateMsa(target_pct=self.target_pct, replicate_id=self.replicate_id),
        }

    def output(self):
        return LocalTarget(os.path.join(self.root_dir, f'fasttree_undecorated_unrooted.tree'))

    def run(self):
        log(f'FastTreeMarkerSplitRandomRunFastTree(pct={self.target_pct}, r={self.replicate_id})', title=True)
