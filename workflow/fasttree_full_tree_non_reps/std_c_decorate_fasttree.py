...
from workflow.fasttree_full_tree_non_reps.std_ba_root_fasttree import FastTreeFullSetNonRepsCreateBatchesStdFastTreeRoot
...
class FastTreeFullSetNonRepsCreateBatchesStandardDecorate(LuigiTask):

    def requires(self):
        return {
            'tree': FastTreeFullSetNonRepsCreateBatchesStdFastTreeRoot(),
            'meta': GtdbMetadataR207()
        }

    def output(self):
        return LocalTargetTree(os.path.join(DIR_OUT_FASTTREE_FULL_TREE_NON_REPS, 'standard',
                                            'phylorank', 'fasttree_decorated.tree'))

    def run(self):
        log(f'FastTreeFullSetNonRepsCreateBatchesStandardDecorate()',
            title=True)
