...
class GtdbMetadataR95(LuigiTask):
    """External GTDB metadata file, available for download from the GTDB website."""

    def output(self):
        return LocalTargetHdf5(os.path.join(DIR_OUT_EXTERNAL, 'gtdb_r95_metadata.h5'))

    def run(self):
        log('Creating GTDB R95 metadata file...', title=True)
        self.make_output_dirs()

        path_ar53 = 'https://data.gtdb.ecogenomic.org/releases/release95/95.0/ar122_metadata_r95.tar.gz'
        ar53_md5 = 'c7dbec00a48e834ab756fa1fed9d5a73'

        path_bac120 = 'https://data.gtdb.ecogenomic.org/releases/release95/95.0/bac120_metadata_r95.tar.gz'
        bac120_md5 = 'ce90da9d0d58756b004b0cd803b925b5'
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            log('Verifying checksums...')
            if md5sum(path_ar53_tmp) != ar53_md5:
                raise ValueError(f'Checksum mismatch for ar53 metadata file: {md5sum(path_ar53_tmp)}')
            if md5sum(path_bac120_tmp) != bac120_md5:
                raise ValueError(f'Checksum mismatch for bac120 metadata file: {md5sum(path_bac120_tmp)}')

            log('Loading metadata...')
            df_ar53 = pd.read_csv(os.path.join(tmpdir, 'ar122_metadata_r95.tsv'), sep='\t', index_col=False,
                                  low_memory=False)
            df_bac120 = pd.read_csv(os.path.join(tmpdir, 'bac120_metadata_r95.tsv'), sep='\t', index_col=False,
                                    low_memory=False)
