...
from workflow.config import DIR_OUT_FASTANI_CONTIG_SPLIT, DIR_OUT_FASTANI_CONTIG_SPLIT_RANDOM
...
class FastAniContigSplitRandomAnalyseFastAni(LuigiTask):
    ...
    def requires(self):
        return {
            'ani': FastAniContigSplitRandomRunFastAni(target_pct=self.target_pct, mash_k=self.mash_k, mash_s=self.mash_s, replicate_id=self.replicate_id),
    ...
    def output(self):
        return LocalTargetHdf5(
            os.path.join(
                self.root_dir, f'results_k{self.mash_k}_s{self.mash_s}__p{self.target_pct}_r{self.replicate_id}.h5'
            ))

    def run(self):
        log(f'FastAniContigSplitRandomAnalyseFastAni(p={self.target_pct}, r={self.replicate_id})', title=True)
