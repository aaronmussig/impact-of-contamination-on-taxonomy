...
from workflow.fastani_contig_split.c_analyse_fastani_relaxed import FastAniContigSplitAnalyseFastAniRelaxed
from workflow.gunc_helper.aggregate_max_css_level_merged import AggregateMaxCssLevelMerged
from workflow.model.luigi import LuigiTask, LocalTargetTsv
from workflow.util.log import log


class FastAniContigSplitReportResultsFastAniRelaxed(LuigiTask):
    ...
    def requires(self):
        return {
            ...
            'sp_results': FastAniContigSplitAnalyseFastAniRelaxed(target_pct=self.target_pct, mash_k=self.mash_k,
                                                                  mash_s=self.mash_s)
        }
    ...
    def output(self):
        return LocalTargetTsv(
            os.path.join(
                self.root_dir,
                f'results_report_k{self.mash_k}_s{self.mash_s}__p{self.target_pct}_relaxed.tsv'
            ))

    def run(self):
        log(f'FastAniContigSplitReportResultsFastAniRelaxed(p={self.target_pct}, mash_k={self.mash_k}, mash_s={self.mash_s})',
            title=True)
