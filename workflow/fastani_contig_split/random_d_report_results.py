...
from workflow.config import DIR_OUT_FASTANI_CONTIG_SPLIT_RANDOM
...
class FastAniContigSplitRandomReportResultsFastAni(LuigiTask):
    ...
    def requires(self):
        return {
            ...
            'sp_results': FastAniContigSplitRandomAnalyseFastAni(target_pct=self.target_pct, mash_k=self.mash_k,
                                                                 mash_s=self.mash_s, replicate_id=self.replicate_id),
        }
    ...
    def run(self):
        log(f'FastAniContigSplitRandomReportResultsFastAni(p={self.target_pct}, r={self.replicate_id})', title=True)
