...
class FastAniContigSplitAnalyseFastAniRelaxed(LuigiTask):
    ...
    def output(self):
        return LocalTargetHdf5(
            os.path.join(
                self.root_dir, f'results_k{self.mash_k}_s{self.mash_s}__p{self.target_pct}_relaxed.h5'
            ))

    def run(self):
        log(f'FastAniContigSplitAnalyseFastAniRelaxed(p={self.target_pct}, mash_k={self.mash_k}, mash_s={self.mash_s})',
            title=True)
        ...
        for gid in tqdm(df_css.index, total=len(df_css.index)):
            ...
            for suffix in ('', '_C'):
                ...
                results.append(
                    run_sp_clustering_on_fastani_values(gid, gid_rep, df_fastani_subset, ANI_RADIUS, gid_suffix))
