...
from workflow.config import MASH_MIN_DIST, DEBUG, DIR_OUT_FASTANI_CONTIG_SPLIT
...
class FastAniContigSplitRunMash(LuigiTask):
    ...
    def output(self):
        return LocalTargetHdf5(os.path.join(
            self.root_dir, f'mash_k{self.mash_k}_s{self.mash_s}__p{self.target_pct}.h5'
        ))

    def run(self):
        log(f'FastAniContigSplitRunMash(p={self.target_pct})', title=True)
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for gid, row in tqdm(df_css.iterrows(), total=len(df_css)):
                ...
                elif row.source == 'progenomes':
                    gunc_source = GuncRefDb.PRO
                else:
                    raise Exception(f'Unknown source: {row.source}')
...
def write_genome_fna_worker(job):
    gid, pct_to_remove, gunc_css, gunc_source, tmp_dir = job

    # Load the genome and split it into the target percent
    genome = Genome(gid)
    try:
        d_contig_to_seq_keep, d_contig_to_seq_disc = genome.split_fna_by_pct(
            max_css=gunc_css, source=gunc_source, pct=pct_to_remove
        )
    except AssertionError:
        print('>>>>', gid, gunc_css, gunc_source)
        raise
