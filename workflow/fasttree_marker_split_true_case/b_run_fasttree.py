...
from workflow.config import DIR_OUT_FASTTREE_MARKER_SPLIT_TRUE_CASE
...
class FastTreeMarkerSplitTrueCaseRunFastTree(LuigiTask):
    ...
    def requires(self):
        return {
            'msa': FastTreeMarkerSplitTrueCaseCreateMsa(target_pct=self.target_pct, replicate_id=self.replicate_id),
        }
    ...
    def run(self):
        log(f'FastTreeMarkerSplitTrueCaseRunFastTree(target_pct={self.target_pct}, r={self.replicate_id})', title=True)
