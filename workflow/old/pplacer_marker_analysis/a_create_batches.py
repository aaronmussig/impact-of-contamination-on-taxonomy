...
class PplacerMarkerAnalysisCreateBatches(LuigiTask):
    # Required for both
    ...
    def output(self):
        ...
        return {
            'msa': LocalTargetFasta(os.path.join(target_dir, 'msa.fasta')),
            'log': LocalTargetHdf5(os.path.join(target_dir, 'log.h5')),
        }

    def load_true_msa(self, keep_gids: Set[str]) -> Dict[str, str]:
        if self.domain == 'bac':
            d_gid_to_msa = self.input()['msa_bac'].maybe_read_cached()
        elif self.domain == 'arc':
            d_gid_to_msa = self.input()['msa_arc'].maybe_read_cached()
        else:
    ...
    def run(self):
        if self.random:
            log(f'RANDOM: PplacerMarkerAnalysisCreateBatches('
                f'pct={self.target_pct}, batch_id={self.batch_id}, '
                f'domain={self.domain})', title=True)
        else:
            log(f'GUNC: PplacerMarkerAnalysisCreateBatches('
                f'congruence={self.congruence}, pct={self.target_pct}, '
                f'domain={self.domain})', title=True)
        self.make_output_dirs()

        # Convert the domain to d__Archaea or d__Bacteria
        domain_str = _domain_to_gtdb_str(self.domain)

        log('Loading metadata')
        df_meta = self.input()['meta'].maybe_read_cached()
        non_rep_gids = set(
            df_meta[
                (df_meta['gtdb_representative'] != 't') &
                (df_meta['domain'] == domain_str)
                ].index
        )
        log(f'Found {len(non_rep_gids):,} non-representative {domain_str} genomes')

        log('Loading MaxCSS')
        df_max_css = self.input()['max_css'].maybe_read_cached()
        fail_gids = set(df_max_css.index)
        log(f'Found {len(fail_gids):,} genomes that failed')
        fail_non_rep_gids = fail_gids.intersection(non_rep_gids)
        log(f'Found {len(fail_non_rep_gids):,} non-representative genomes that failed')

        if self.random:
            log('Subset dataframe a random selection of non-representative genomes')
            gids_to_run_on = random.sample(non_rep_gids, len(fail_non_rep_gids))
        else:
            gids_to_run_on = fail_non_rep_gids
            log(f'Found {len(gids_to_run_on):,} non-representative that failed for {self.domain}')

            log('Subset dataframe to only those fail non rep gids')
            df_max_css_subset = df_max_css[df_max_css.index.isin(gids_to_run_on)]
            log(f'DF Length: {len(df_max_css_subset):,}')
        ...
        log('Creating queue')
        if self.random:
            queue = create_queue_random(gids_to_run_on, self.target_pct)
        else:
        ...
        with mp.Pool(processes=int(mp.cpu_count()) if not DEBUG else 1) as pool:
            if self.random:
                results = list(tqdm(pool.imap_unordered(work_queue_random, queue), total=len(queue), smoothing=0.01))
            else:
