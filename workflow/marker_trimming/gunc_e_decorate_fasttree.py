...
class MarkerTrimmingGuncDecorateFastTree(LuigiTask):
    ...
    def output(self):
        return LocalTargetTree(os.path.join(self.root_dir, 'phylorank', 'fasttree_rooted_decorated.tree'))

    def run(self):
        log(f'MarkerTrimmingGuncDecorateFastTree(pct={self.target_pct})', title=True)
