...
from workflow.marker_trimming.gunc_b_run_fasttree import MarkerTrimmingGuncRunFastTree
from workflow.model.luigi import LuigiTask
from workflow.util.log import log


class MarkerTrimmingGuncRootFastTree(LuigiTask):
    ...
    def requires(self):
        return {
            'tree': MarkerTrimmingGuncRunFastTree(target_pct=self.target_pct, replicate_id=self.replicate_id),
    ...
    def run(self):
        log(f'MarkerTrimmingGuncRootFastTree(pct={self.target_pct}, replicate={self.replicate_id})', title=True)
